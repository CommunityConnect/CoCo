...
public class TopologyService {
	...
	private List<NetworkInterface> getNetworkInterfaces() {
		...
		List<NetworkInterface> if_bgp = topologyDao.getNetworkInterfaces_BGP();
	...
	public String getTopologyJsonVis() {
		...
		for (NetworkElement networkElement : nodeSet) {
			...
			if (networkElement.nodeType.equals(NetworkElement.NODE_TYPE.CUSTOMER)) {
				...
				fakeId = networkElement.getId();
			} else if (networkElement.nodeType.equals(NetworkElement.NODE_TYPE.CUSTOMER_BGP)){
				fakeId = 300 + networkElement.getId();
			}
